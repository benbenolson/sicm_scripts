#!/bin/bash
# This is a collection of functions that benchmarks can use to define common environment variables to compile their applications.
# One example is, to compile LULESH, the `benchmarks/lulesh/lulesh_build.sh` script calls `bench_build c`.

# Arguments. Only one: `--bench=[name of bench to build]`.
GETOPT_OUTPUT=`getopt -o b --long bench:, -n 'args.sh' -- "$@"`
